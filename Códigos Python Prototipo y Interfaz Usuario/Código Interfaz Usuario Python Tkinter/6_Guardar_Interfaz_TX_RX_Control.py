...
# Definir ser como una variable global
ser = None
...
# Configurar la columna 2 para la nueva entrada y el botón de enviar
root.grid_columnconfigure(2, weight=1)  # Añadir configuración de columna para los elementos de la derecha
...
led_canvas.create_text(100, 50, text="Frío")
led_canvas.create_text(140, 50, text="Caliente")
...
boton_inicio.grid(row=3, column=1, columnspan=2)
...
boton_limpiar.grid(row=3, column=2, columnspan=2)


# Nueva entrada para enviar datos
etiqueta_enviar_datos = Label(root, text="Temperatura de Referencia:")
etiqueta_enviar_datos.grid(row=5, column=1, sticky=tk.E)  # Posición ajustada
entrada_enviar_datos = Entry(root)
entrada_enviar_datos.grid(row=5, column=2, sticky=tk.W)  # Posición ajustada

# Función para enviar datos a través del puerto serie
def enviar_datos():
    global ser  # Asegurarse de usar la variable global ser
    datos = entrada_enviar_datos.get()
    if ser is not None and ser.isOpen():
        ser.write(datos.encode())

# Botón para enviar datos
boton_enviar_datos = Button(root, text="Enviar", command=enviar_datos)
boton_enviar_datos.grid(row=6, column=1, sticky=tk.E)
